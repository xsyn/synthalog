(ns synthalog.tutorials.sampled-flute
  (:require [overtone.live :refer :all]))

(def FREESOUND-VIBRATO-FLUTE-SAMPLES
  "Freesound ids for all samples in Vibrato Transverse Flute pack - https://www.freesound.org/search/?q=flute&f=grouping_pack%3A%229549_Transverse+Flute%3A+Tenuto+Vibrato+C4-C7"
...
;; Convert freesound id into a midi note

(defn- buffer->midi-note [buf]
  (-> buf :freesound-id FREESOUND-VIBRATO-FLUTE-SAMPLES name note))

;; Generate buffer id to midi-note values

(defn- note-index [buffers]
...
;; Make sure all 127 buffer spaces are filled and set buffers
